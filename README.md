...
`joinr` and allows joins to be performed via IDs stored by themselves (`byOne`) or in an array property ("byArray"). Joins can be performed when the ID of the related document is in the document you already have ("byOne" or "byArray") and also when the related documents contain the IDs of documents you already have ("byOneReverse" and "byArrayReverse").
